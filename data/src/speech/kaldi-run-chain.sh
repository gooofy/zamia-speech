...
if [ $stage -le 2 ]; then
    ...
    cat lm/lm.arpa | utils/find_arpa_oovs.pl data/lang_test/words.txt  > data/local/lm/oovs_lm.txt
    ...
    cat lm/lm.arpa | \
        ...
        grep -v '</s> </s>' | \
        arpa2fst - | fstprint | \
        utils/remove_oovs.pl data/local/lm/oovs_lm.txt | \
...
#
# smaller model for embedded use without ivector
#

if [ $stage -le 11 ]; then

    dir=exp/nnet3${nnet3_affix}/tdnn_250_no_ivector
